...
namespace Private {
  ...
  export async function hint(
    ...
    // to the matches...
    // TODO: pull request which:
    //  - changes the text+metadata into list of objects obeying an interface
    //  - the interface allows to pass an optional score (number)
    //  - if the score is set, the _filter does not recalculate it (https://github.com/jupyterlab/jupyterlab/blob/3592afd328116a588e3307b4cdd9bcabc7fe92bb/packages/completer/src/model.ts#L373
    //  - the interface also allows to provide documentation, which is not used by default
    //  - but the renderer is modified so that it can be customized (making use of documentation)
